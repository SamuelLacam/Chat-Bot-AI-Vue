...
const promptSubmit = inject("promptSubmit") as (
  inputMessage: string,
  reply: (convId: number, messageId: number) => Promise<number>,
) => Promise<number>;
...
<template>
  <div class="prompt-input-container" @click="inputRef.focus()">
    <button class="add-image-btn" @click.stop="">
      <svg
        width="20"
        height="20"
        viewBox="1 1 18 18"
    ...
    <form class="prompt-form" @submit.prevent="submitHandle">
      <input ref="inputRef" type="text" class="prompt-input" placeholder="Ask a question" />
      <button v-if="!isReplying" type="submit" class="send-prompt-btn" @click.stop>
        <svg
          width="20"
          height="20"
          viewBox="-1 -1 22 22"
      ...
      <button v-else class="send-prompt-btn" @click.stop="chatsStore.abortReply(convId)">
